...
class _ParentDashboardScreenState extends State<ParentDashboardScreen> {
  ...
  Map<DateTime, int> _calculateWeeklyProgress(List<dynamic> progress) {
    ...
    for (var p in progress) {
      ...
      weeklyProgress[weekStart] =
          ((weeklyProgress[weekStart] ?? 0) + completedSteps).toInt();
    }
  ...
  String _getSuggestion() {
    ...
    final subjects = _achievements
        .map((a) => a['experimentId']?['subject'] ?? 'Other')
        .toSet()
        .toList();
    final subjectCounts = subjects
        .map(
          (s) => _achievements
              .where((a) => a['experimentId']?['subject'] == s)
              .length,
        )
        .toList();
    ...
    if (subjects.isNotEmpty) {
      ...
      recommendation =
          'Try more $leastExplored experiments to diversify their learning.';
    }
  ...
  Widget build(BuildContext context) {
    debugPrint(
      'Building ParentDashboardScreen: _isLoading=$_isLoading, _progress=${_progress.length}, _achievements=${_achievements.length}',
    );
    return Scaffold(
        ...
        actions: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              color: _isSpeaking
                  ? null
                  : const Color(0xFF562866).withOpacity(0.08),
              borderRadius: BorderRadius.circular(12),
            ...
            child: IconButton(
              icon: Icon(
                ...
                color: _isSpeaking ? Colors.white : const Color(0xFF562866),
                size: 22,
              ),
              onPressed: _speakSummary,
              tooltip: _isSpeaking ? 'Stop Audio' : 'Listen to Summary',
            ),
          ),
        ],
      ),
                  ...
                  children: [
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: DropdownButton<String>(
                              ...
                              items: ['All', 'Completed', 'In Progress'].map((
                                String value,
                              ) {
                            ...
                            child: _progress.isEmpty
                                ? Center(
                                    child: Text(
                                      ...
                                      style: TextStyle(color: _disabledColor),
                                    ),
                                  )
                                : PieChart(
                                    PieChartData(
                    ...
                    if (_progress.isEmpty)
                      ...
                      ...(_filter == 'All'
                          ...
                          .map((prog) {
                            final totalSteps =
                                prog['experimentId']?['steps']?.length ?? 0;
