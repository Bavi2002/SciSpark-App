...
// Read teacher-specific experiments
router.get("/teacher/:id", auth, async (req, res) => {
  try {
    const experiments = await Experiment.find({ user: req.params.id }).select(
      "-steps"
    );
    res.json(experiments);
  } catch (error) {
...
router.post("/add", auth, authorizeRole(["teacher"]), async (req, res) => {
  try {
    const { title, description, subject, difficulty, materials, steps } =
      req.body;
...
// Create a step to an experiment (Teacher only, requires auth)
router.post(
  "/:id/steps",
  auth,
  ...
  async (req, res) => {
    try {
      ...
      if (!stepNumber || !instruction) {
        return res
          .status(400)
          .json({ message: "Step number and instruction are required" });
      }
      const experiment = await Experiment.findById(req.params.id);
      if (!experiment) {
        return res.status(404).json({ message: "Experiment not found" });
